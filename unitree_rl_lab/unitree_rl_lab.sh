...
case "$1" in
    -i|--install)
        ...
        if command -v activate-global-python-argcomplete >/dev/null 2>&1; then
            activate-global-python-argcomplete --user || echo "[Warn] Skipped global argcomplete installation."
        else
            echo "[Warn] activate-global-python-argcomplete not found; skipped shell completion installation."
        fi
        ;;
    -l|--list)
        shift
